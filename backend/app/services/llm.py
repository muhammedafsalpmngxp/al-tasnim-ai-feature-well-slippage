...
def summarize_well(ai_evidence):
    ...
    prompt = (
        ...
        "Write one paragraph in this order when supported: (1) stage "
        "and the date establishing it; (2) every milestone with "
        "delay_days > 0 -- gate, expected date, actual date or 'not "
        "recorded', exact delay; (3) early/on-schedule milestones when "
        "useful, never as delayed; (4) the well-level delay and its "
        "gate; (5) accountability classification and recorded reason, "
        "kept separate from causality; (6) most delayed activities "
        "with their own delay values, kept separate from the "
        "well-level figure; (7) what the remarks record, never as a "
        "cause; (8) projects if useful; (9) the data-quality caveat "
        "and its flags. State each fact once; skip unsupported points.\n\n"
